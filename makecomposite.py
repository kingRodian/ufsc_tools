...
class CompositeGenerator:
    def __init__(self, args, files):
        ...
        self.totalsize = self.totalwidth, self.totalheight = self.width * \
            self.cellwidth, self.height * self.cellheight
    ...
    def create_composite(self):
        try:
            for i, filename in enumerate(self.files):
                ...
                pastepos = (cellpos[0] * self.cellwidth,
                            cellpos[1] * self.cellheight)
    ...
    def save_composite(self):
        if self.outfile:
            ...
            filename = 'composite_{}_{}x{}_{}x{}.png'.format(
                self.mode, self.width, self.height, self.cellwidth, self.cellheight)
...
def main():
    ...
    files = [filename for filename in os.listdir(
        args.inputdir) if file_expr.search(filename)]
